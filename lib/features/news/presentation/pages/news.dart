...
class _NewsState extends State<News> {
  ...
  buildArticle(NewsLoadedState state, int index) {
    return InkWell(
      onTap: () {
        //Navigator.pushNamed(context, Routes.newsDetailsRoute,arguments:state.news[index] );
        Routes.newsDetailsRoute
            .pushWithData({NewsDetails.modelKey: state.news[index]});
        //Navigator.push(context, MaterialPageRoute(builder:(context)=>NewsDetails( model: state.news[index],)));
      },
      child: Padding(
        padding: EdgeInsets.symmetric(horizontal: 16.w),
        child: Stack(
          children: [buildListTile(state, index), buildCalender(state, index)],
        ),
      ),
    );
  }
  ...
  buildCalender(NewsLoadedState state, int index) {
    return Positioned(
      ...
      left: 220.w,
      child: Wrap(
        children: [buildIconCalender, buildPublishedDate(state, index)],
      ),
    );
  }
    ...
    return SizedBox(
      ...
      child: Row(
        children: [
          ...
          15.hs,
          buildListTileTitle(
              state.news[index].title!, state.news[index].byline!),
          const Spacer(),
          buildReverseIcon
        ],
      ),
  ...
  buildCircleImage(NewsLoadedState state, int index) {
    return Container(
      ...
      decoration: BoxDecoration(
          ...
          image: state.news[index].media?.isNotEmpty == true && state.news[index].media![0].mediaMetadata?.isNotEmpty == true ? DecorationImage(
            image: NetworkImage(
              state.news[index].media?[0].mediaMetadata?[0].url! ?? '',
            ),
          ) : null),
    );
  }
  ...
  get buildIconCalender => const Icon(
  ...
  buildPublishedDate(NewsLoadedState state, int index) {
    return Text(
      state.news[index].publishedDate!,
      style: getRegularTextStyle(
          fontSize: 10,
